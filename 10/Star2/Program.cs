class Map
{
    ...
    public int getScore(int x, int y, int num)
    {
        int score = 0;
        ...
        {
            if (map[y, x + 1] == num + 1)// if next number is to the right
            {
                score += getScore(x + 1, y, num + 1);
            }
        }
        if (y < map.GetLength(0) - 1)   // if y+1 is not out of bounds 
        {
            if (map[y + 1, x] == num + 1)// if next number is down
            {
                score += getScore(x, y + 1, num + 1);
            }
        }
        if (x > 0)   // if x-1 is not out of bounds 
        {
            if (map[y, x - 1] == num + 1)// if next number is left
            {
                score += getScore(x - 1, y, num + 1);
            }
        }
        ...
        {
            if (map[y - 1, x] == num + 1)// if next number is up
            {
                score += getScore(x, y - 1, num + 1);
            }
        }
        ...
        for (int i = 0; i < map.GetLength(0); i++)
        {
            for (int j = 0; j < map.GetLength(1); j++)
            {
                if (map[i, j] == 0) //for each trailhead
                {
                    ...
                    trailheadScores.Add(getScore(j, i, 0));
                }
            }
        }
